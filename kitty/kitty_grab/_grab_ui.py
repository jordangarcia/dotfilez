...
class GrabHandler(Handler):
    ...
    def escape(self, *args: Any) -> None:
        if self.mode != 'normal' and self.mark is not None:
    ...
    def word_left(self) -> Position:
        if self.point.x > 0:
            ...
            # skip non-word chars (whitespace/separators) going left
            non_word = ''.join(takewhile(
                lambda c: not self._is_word_char(c), reversed(line[:pos])))
            pos -= len(non_word)
            if pos > 0:
                # skip word chars going left to land at start of word
                word = ''.join(takewhile(
                    self._is_word_char, reversed(line[:pos])))
                pos -= len(word)
            return Position(wcswidth(line[:pos]),
                            self.point.y, self.point.top_line)
        if self.point.y > 0:
    ...
    def word_right(self) -> Position:
        ...
        if pos < len(line):
            ...
            pos += len(word)
            # skip non-word chars (whitespace/separators) to land on next word
            non_word = ''.join(takewhile(
    ...
    def word_end(self) -> Position:
        line = unstyled(self.lines[self.point.line - 1])
        pos = truncate_point_for_length(line, self.point.x)
        # advance one char to escape current word boundary
        pos += 1
        if pos < len(line):
            # skip non-word chars (whitespace/separators)
            non_word = ''.join(takewhile(
                lambda c: not self._is_word_char(c), line[pos:]))
            pos += len(non_word)
            if pos < len(line):
