...
function M.statuscolumn()
  ...
  if show_signs then
    ...
    components[1] = M.icon(diag and { text = icons[diag.name], texthl = diag.name } or {})
    components[3] = is_file and M.icon(git) or ""
  end
  ...
  if (is_num or is_relnum) and vim.v.virtnum == 0 then
    if vim.v.relnum == 0 then
      components[2] = "%#StatusColumnNr#" .. (is_num and "%l" or "%r") -- the current line
    else
      components[2] = is_relnum and "%r" or "%l" -- other lines
    end
    components[2] = "%#StatusColumn#%=" .. components[2] .. " " -- right align
  end
